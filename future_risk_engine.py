...
INPUT_PATH = (
    "model_outputs_multi/calibration/"
    "multi_target_calibrated_predictions.csv"
)

OUTPUT_DIR = "model_outputs_multi/risk_engine"
...
UTILIZATION = (
    "FUTURE_HIGH_UTILIZATION_CALIBRATED_PERCENT"
)

DETERIORATION = (
    "FUTURE_CLINICAL_DETERIORATION_CALIBRATED_PERCENT"
)

ESCALATION = (
    "FUTURE_HEALTHCARE_ESCALATION_CALIBRATED_PERCENT"
)

# ============================================================
# IMPORTANT:
# High Utilization calibration performed slightly worse.
# Therefore use ORIGINAL probability for this target.
# ============================================================

df["CURRENT_RISK_PROBABILITY"] = (
    df["CURRENT_RISK_CALIBRATED_PERCENT"]
)

UTILIZATION_ORIGINAL = "FUTURE_HIGH_UTILIZATION_ORIGINAL_PROB"

df["UTILIZATION_PROBABILITY"] = (
        df[UTILIZATION_ORIGINAL]
        * 100
)

df["DETERIORATION_PROBABILITY"] = (
    df[DETERIORATION]
)
...
def highest_risk_type(row):
    values = {
        ...
        "HIGH_UTILIZATION":
            row["UTILIZATION_PROBABILITY"],

        "CLINICAL_DETERIORATION":
            row["DETERIORATION_PROBABILITY"],

        "HEALTHCARE_ESCALATION":
...
# 50% is a presentation threshold.
# It is NOT a medical decision threshold.

df["ELEVATED_RISK_DOMAINS"] = (

        (df["UTILIZATION_PROBABILITY"] >= 50)
        .astype(int)

        +

        (df["DETERIORATION_PROBABILITY"] >= 50)
        .astype(int)

        +

        (df["ESCALATION_PROBABILITY"] >= 50)
        .astype(int)
...
df["RISK_PROFILE"] = (
    df.apply(overall_risk_profile, axis=1)
)
...
# ============================================================
# STEP 8 — SAVE COMPLETE PREDICTION DATA
# ============================================================

complete_output = (
    f"{OUTPUT_DIR}/"
    "future_risk_engine_complete.csv"
)
