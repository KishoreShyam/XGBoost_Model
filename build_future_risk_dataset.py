...
riskscore_df = pd.read_csv("DataSet/Synthea_Dataset_with_RiskScore.csv", usecols=["PATIENT_ID", "LABEL"], low_memory=False)
...
# Define CURRENT_RISK target
df["CURRENT_RISK"] = df["LABEL"].isin(["High", "Very High"]).astype(int)
print("CURRENT_RISK", df["CURRENT_RISK"].value_counts(normalize=True).round(3).to_dict())
