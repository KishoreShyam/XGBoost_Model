...
existing_current = [c for c in current_features if c in df.columns]
if existing_current:
...
correlations = df[numeric_columns].corr()["LABEL"].drop("LABEL").abs().sort_values(ascending=False)
...
pairs = [
    ("PAST_ENCOUNTER_COUNT_12M", "CURRENT_ENCOUNTER_COUNT_12M"),
    ("PAST_CONDITION_COUNT_12M", "CURRENT_CONDITION_COUNT_12M"),
    ("PAST_MEDICATION_COUNT_12M", "CURRENT_MEDICATION_COUNT_12M"),
    ("PAST_PROCEDURE_COUNT_12M", "CURRENT_PROCEDURE_COUNT_12M"),
    ("PAST_OBSERVATION_COUNT_12M", "CURRENT_OBSERVATION_COUNT_12M"),
]
...
for past, current in pairs:
    ...
    difference = df[current] - df[past]
    print(f"\n{past} → {current}")
    print("Mean change:", round(difference.mean(), 3))
    print("Median change:", round(difference.median(), 3))
    print("Current > Past:", round((difference > 0).mean() * 100, 2), "%")
    print("Current = Past:", round((difference == 0).mean() * 100, 2), "%")
    print("Current < Past:", round((difference < 0).mean() * 100, 2), "%")
...
if "PATIENT_ID" in df.columns:
    ...
    print("Patients with >1 row:", (patient_counts > 1).sum())
    print("Maximum rows per patient:", patient_counts.max())
...
for column in existing_current:
    print("\n", column)
    for label in [0, 1]:
        values = df.loc[df["LABEL"] == label, column]
        print(f"LABEL {label}: mean={values.mean():.3f}, median={values.median():.3f}, min={values.min():.3f}, max={values.max():.3f}")
