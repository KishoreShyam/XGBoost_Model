...
print("=" * 70)
print(f"TRAINING CURRENT RISK MODEL  (target = {target_col})")
print("=" * 70)
...
importance_df.to_csv(os.path.join(MODEL_DIR, "shap_importance_current_risk.csv"), index=False)
print("Top 5 SHAP features:", importance_df.head(5)["feature"].tolist())

joblib.dump(model, os.path.join(MODEL_DIR, "xgb_current_risk.pkl"))
joblib.dump(final_model, os.path.join(MODEL_DIR, "final_current_risk.pkl"))
joblib.dump(imputer, os.path.join(MODEL_DIR, "imputer_current_risk.pkl"))
joblib.dump(use_calibrated, os.path.join(MODEL_DIR, "use_calibrated_current_risk.pkl"))
