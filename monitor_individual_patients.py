...
BASELINE_DATA_PATH = "DataSet/future_model/Cognizant_14K_Model_Ready.csv"
...
# Select key features to monitor for outliers
...
# Calculate means and std devs from training set
feature_stats = {}
...
def monitor_dataset(data_path, label):
    ...
    # Fill/impute features
    X = df[features].copy().replace([np.inf, -np.inf], np.nan)
    
    # Predict risk probabilities for all outcomes
    ...
    # ------------------------------------------------------------
    # Group by PATIENT_ID, sort by SNAPSHOT_DATE to calculate temporal diffs
    df = df.sort_values(by=["PATIENT_ID", "SNAPSHOT_DATE"]).reset_index(drop=True)
    
    spike_flags = []
    spike_details = []
    
    # Group patient records and calculate successive jumps
    grouped = df.groupby("PATIENT_ID")
    
    # We map patient and index to find diffs easily
    diff_records = {}
    for pid, group in grouped:
        if len(group) > 1:
            # Sort by snapshot date to calculate successive differences
...
# Monitor both the baseline model ready dataset (test split) and the drifted dataset
test_results = monitor_dataset(BASELINE_DATA_PATH, "Test Dataset")
drifted_results = monitor_dataset("DataSet/future_model/Cognizant_14K_Drifted.csv", "Drifted Dataset")
